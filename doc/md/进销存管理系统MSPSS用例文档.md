...
<a href="#13">     用例5 制定库存报损报溢单</a><br>
<a href="#14">     用例6  制定库存赠送单</a><br>
...
| 韩新虎      | 2017-09-29 | 发现用例描述不明    | v1.1.0    |
| 徐光耀      | 2017-10-06 | 增加用例        | v1.1.1    |
| 伏家兴      | 2017-10-12 | 修改批改中的一些问题  | v1.1.2    |
| 韩新虎      | 2017-10-14 | 订正助教说明的一些问题 | v1.1.3    |
| 韩新虎      | 2017-10-19 | 再次改正助教指出的问题 | v1.1.4    |
...
| **后置条件**    | 系统存储被修改的商品分类信息，并且更新存储。                   |
| **优先级**     | 高                                        |
| **正常流程**    | 1.增加分类<br>  1.1库存管理人员选择增加分类<br>  1.2库存管理人员选择增加分类的位置<br>1.3系统提示输入商品名称 <br> 1.4库存管理人员输入名称并确认增加分类<br>1.5系统提示增加分类成功并为该分类生成一个编号，更新分类信息<br>2.删除分类<br>  2.1库存管理人员选择删除分类<br>  2.2 选择要删除的分类<br>  2.3系统提示是否要删除该分类，操作不可逆<br>  2.4确认删除，系统删除该分类的编号,否则返回上一级<br>  3.修改分类<br>  3.1选择修改分类<br>  3.2选择修改类型<br>  3.3.1选择修改分类名称<br>3.3.2系统提示输入新名称<br>3.3.3库存管理输入新名称并确认<br>3.3.4系统提示修改完成并更新分类信息<br>3.4.1 选择修改分类位置<br>3.4.2选择分类放置的节点<br> 3.4.3确认操作<br> |
| **扩展流程**    | 1.2a该位置下已经存在商品<br>  1.系统提示该位置已存在商品，该节点不可选择<br>2.返回上一操作界面<br>1.3a  系统中已经存在该分类<br>1.系统提示已经存在该分类<br>2.返回上一操作界面<br>1.5a分类信息有误<br>1.提示重新输入或取消该操作<br>2.2a该位置下已经存在商品或者子分类<br>  1.系统提示该分类下存在商品和子分类，无法选择<br> |
| **特殊需求**    | 分类的名称长度小于16                              |

### 用例2 商品管理<a name="10"></a>
...
| **前置条件**    | 库存管理人员以及被识别和授权。                          |
| **后置条件**    | 系统存储修改后的商品信息，并且更新存储。                     |
| **优先级**     | 高                                        |
| **正常流程**    | 1.增加商品<br>  1.1库存管理人员选择增加商品<br>  1.2系统要求输入商品基本信息，包括 名称、型号、库存数量、进价、零售价<br>  1.3库存管理人员输入完成保存<br>  1.4选择商品分类<br>1.5系统显示添加成功，并且商品根据其所在的分类以及添加次序自动生成商品编号并更新存储信息<br>2.删除商品<br>  2.1库存管理人员选择删除商品<br>  2.2库存管理人员输入关键字<br>  2.3系统显示所有相关商品信息（编号、名称、型号、库存数量、进价、零售价、最近进价、最近零售价）<br>  2.4库存管理人员点击删除<br>  2.5系统提示是否确认删除<br>  2.6库存管理人员确认删除<br>  2.7系统更新存储并显示删除成功<br>3.修改商品<br>  3.1库存管理人员选择修改商品<br>  3.2系统要求输入编号或名称或型号<br>  3.3库存管理人员输入关键字<br>  3.4系统显示满足关键字的商品列表<br>  3.5库存管理人员选择要修改的商品<br>  3.6系统显示该商品的信息<br>  3.7库存管理人员根据需要修改，修改完成保存<br>  3.8系统提示修改完成,并更新存储<br>4.查询商品信息<br>  4.1库存管理人员选择查询商品信息<br>  4.2系统要求输入编号或名称或型号<br>  4.3库存管理人员输入关键字<br>  4.4系统显示满足关键字的商品列表<br>  4.5库存管理人员选择要查询信息的商品<br>  4.6系统显示该商品的信息<br> |
| **扩展流程**    | 1..3a商品信息残缺或格式不正确<br>1.提示重新输入或取消添加<br>2.4a/3.4a/4.4a 未找到满足关键字的商品<br>  1.系统提示重新输入或者取消操作<br>1.3a商品已存在<br>  1.系统提示商品已存在，添加失败<br>2.5a商品已经存在记录无法删除<br>2.7a 需要取消删除<br>  1.库存管理人员选择放弃删除<br>  2.系统取消操作<br>3.7a 修改导致信息重复<br>  1.系统提示信息重复，修改失败 |
| **特殊需求**    | 1.开始规定一个进价和售价作为默认值,保存一个最近的进价和售价属性<br>2.商品编号根据其所在的分类以及添加次序自动生成<br>3.商品的查询可以通过输入关键字，商品编号等进行模糊查找<br> |

## 用例3 库存查看<a name="11"></a>
...
| **后置条件**    | 更新库存信息                                   |
| **优先级**     | 高                                        |
| **正常流程**    | 1.库存管理人员收到提醒产生库存报警单的提醒<br>2.库存管理人员选择要查看的库存报警单 <br> 3. 库存管理人员选择对库存报警单进行增加备注信息<br> 4.操作完成，系统返回原来的界面 |
| **扩展流程**    | 无                                        |
| **特殊需求**    | 无                                        |

## 用例8 客户管理<a name="16"></a>

| **ID**   | 8                                        | **名称**      | 客户管理       |
| -------- | ---------------------------------------- | ----------- | ---------- |
| **创建者**  | 伏家兴                                      | **最后一次更新者** | 伏家兴        |
| **创建日期** | 2017/9/14                                | **最后更新日期**  | 2017/10/12 |
| **参与者**  | 进货销售人员，目标是正确快速地完成对客户信息的操作                |             |            |
| **触发条件** | 有客户信息需要改动                                |             |            |
| **前置条件** | 进货销售人员必须已经被识别和授权。                        |             |            |
| **后置条件** | 更新系统中的客户属性，存储日志记录（包括操作者、操作时间和操作）         |             |            |
| **优先级**  | 高                                        |             |            |
| **正常流程** | 1.查询客户<br />   1.1 输入关键字（包括编号、分类（进货商、销售商）、级别、姓名、电话、地址、邮编、电子邮箱、应收额度、应收、应付、默认业务员。） <br />  1.2 系统搜索符合关键字信息的客户  <br /> 1.3 系统显示符合搜索条件的所有客户 <br />  2.增加客户<br />   2.1 输入客户的编号、分类（进货商、销售商）、级别（五级，一级普通用户，五级VIP客户）、姓名、电话、地址、邮编、电子邮箱、应收额度、应收、应付、默认业务员。（应收应付不可修改）<br />   2.2 从系统中查找该客户，如果不存在此用户，则添加一则用户信息。 <br />  3．删除客户： <br />  3.1 输入要删除的客户的编号，查询此客户<br />   3.2 如果编号一致，则删除此用户的所有客户信息。 <br />  4．修改客户属性： <br />  4.1 通过关键字搜索或者编号信息来查询客户<br />   4.2 如果查询到用户，对其需要修改的属性进行修改（应收应付不可修改） |             |            |
| **扩展流程** | 1.3a. 系统中没有查询的客户<br />   1. 系统显示不存在该客户，并提示重新查找  <br /> 2.1a. 输入的客户属性有缺少<br />   1. 系统提示缺少信息，提示操作人员核对信息重新输入<br />   2.2a.  系统已经存在要增加的客户的信息 <br />  1. 系统显示客户已存在，并提示核对信息重新添加。 <br />  3.1a.   系统不存在要删除的客户的编号 <br />  1. 提示该用户不存在并提示操作人员核对信息重新输入  <br /> 4.1a.   查询不到要查找的客户 <br />  1. 提示该用户不存在并提示操作人员核对信息重新输入   <br />4.2a. 操作人员对应收应付信息进行修改<br />   1. 提示该信息不可修改 |             |            |
| **特殊需求** |                                          |             |            |



## 用例9 制定进货单<a name="17"></a>

| **ID**   | 9                                        | **名称**      | 制定进货单      |
| -------- | ---------------------------------------- | ----------- | ---------- |
| **创建者**  | 伏家兴                                      | **最后一次更新者** | 伏家兴        |
| **创建日期** | 2017/9/14                                | **最后更新日期**  | 2017/10/20 |
| **参与者**  | 进货销售人员，目标是正确快速的制定进货单                     |             |            |
| **触发条件** | 有需要处理的进货单                                |             |            |
| **前置条件** | 进货退货人员必须已经被识别和授权。                        |             |            |
| **后置条件** | 存储制定进货单的日志，包括操作人、操作时间和操作。                |             |            |
| **优先级**  | 高                                        |             |            |
| **正常流程** | 1. 进货销售人员选择制定进货单。<br />        2. 新建一个进货单包括：包括：**单据编号** ，**供应商，仓库，操作员，入库商品列表，备注，总额合计。**其中入库商品列表包含的信息有：**商品编号，名称（从商品选择界面进行选择），型号，数量，单价（默认为商品信息中的进价），金额，备注**。<br />  3. 进货销售人员确认生成单据。<br />4.系统生成单据，并提示新建成功 |             |            |
| **扩展流程** | 2a.   操作人员遗漏掉某些需要处理的货物信息  <br />        1.  系统提示有信息未填，并督促操作人员进行操作<br>4a.   需要生成的进货单数量超过最大范围（即超过99999条）<br />     1.  系统报错，提示已达到每日生成进货单的最大数量 |             |            |
| **特殊需求** | 1.**单据编号**格式为：JHD-yyyyMMdd-xxxxx，后五位每天从1开始编号，所以一天最多可以生成99999条单子<br>2.需要手动输入的信息有数量，备注 |             |            |



## 用例10 制定进货退货单<a name="18"></a>
...
| -------- | ---------------------------------------- | ----------- | ---------- |
| **创建者**  | 伏家兴                                      | **最后一次更新者** | 伏家兴        |
| **创建日期** | 2017/10/20                               | **最后更新日期**  | 2017/10/20 |
| **参与者**  | 进货销售人员，目标是正确快速的制定进货退货单                   |             |            |
| **触发条件** | 有需要处理的进货退货单                              |             |            |
| **前置条件** | 进货退货人员必须已经被识别和授权。                        |             |            |
| **后置条件** | 存储制定进货退货单的日志，包括操作人、操作时间和操作。              |             |            |
| **优先级**  | 高                                        |             |            |
| **正常流程** | 1. 进货销售人员选择制定进货退货单。<br />        2. 新建一个进货退货单，单据信息包括：**单据编号** ，**供应商，仓库，操作员，出库商品列表，备注，总额合计。**其中出库商品列表包含的信息有：**商品编号，名称（从商品选择界面进行选择），型号，数量，单价（默认为商品信息中的进价），金额，备注**。<br />   3. 进货销售人员确认生成单据。<br />4.系统生成单据，并提示新建成功 |             |            |
| **扩展流程** | 2a.   操作人员遗漏掉某些需要处理的货物信息  <br />        1.  系统提示有信息未填，并督促操作人员进行操作<br>4a.   需要生成的进货退货单数量超过最大范围（即超过99999条）<br />     1.  系统报错，提示已达到每日生成进货退货单的最大数量 |             |            |
| **特殊需求** | 1.**单据编号**格式为：JHTHD-yyyyMMdd-xxxxx，后五位每天从1开始编号，所以一天最多可以生成99999条单子<br>2.需要手动输入的信息有数量，备注<br> |             |            |



## 用例11 制定销售单<a name="19"></a>

| **ID**   | 11                                       | **名称**      | 制定销售单      |
| -------- | ---------------------------------------- | ----------- | ---------- |
| **创建者**  | 伏家兴                                      | **最后一次更新者** | 伏家兴        |
| **创建日期** | 2017/9/14                                | **最后更新日期**  | 2017/10/12 |
| **参与者**  | 进货销售人员，目标是正确快速的制定销售单                     |             |            |
| **触发条件** | 有需要处理的销售单                                |             |            |
| **前置条件** | 进货销售人员必须已经被识别和授权。                        |             |            |
| **后置条件** | 存储销售单的日志，包括操作人、操作时间、操作。                  |             |            |
| **优先级**  | 高                                        |             |            |
| **正常流程** | 1.  选择制定销售单。<br />       2. 新建一个销售单包括：包括：**单据编号**，**客户**，**业务员**，**操作员**，**仓库，出货商品清单，折让前总额，折让，使用代金卷金额，折让后总额，备注**。出货商品清单中要显示商品的**编号，名称，**型号**，数量**，单价**（默认为商品信息里的销售价，可修改）**，金额（自动生成），商品备注。3.财务人员从商品选择界面选择名称<br>4.系统自动生成金额<br>5. 进货销售人员确认生成单据。<br />6.系统生成单据，并提示新建成功 |             |            |
| **扩展流程** | 2a.   操作人员遗漏掉某些需要处理的货物信息  <br />1.  系统提示有信息未填，并督促操作人员进行操作<br> 4a.   需要生成的单子超过最大范围（即超过99999条）<br />   1.  系统报错，提示已达到每日生成销售单的最大数量 |             |            |
| **特殊需求** | 1.销售单单据格式为XSD-yyyyMMdd-xxxxx，后五位每天从1开始编号，所以一天最多可以生成99999条单子<br>2.需要手动输入的信息有数量和商品备注。 |             |            |



## 用例12 制定销售退货单<a name="20"></a>

| **ID**   | 12                                       | **名称**      | 制定销售单      |
| -------- | ---------------------------------------- | ----------- | ---------- |
| **创建者**  | 伏家兴                                      | **最后一次更新者** | 伏家兴        |
| **创建日期** | 2017/10/20                               | **最后更新日期**  | 2017/10/20 |
| **参与者**  | 进货销售人员，目标是正确快速的制定销售退货单                   |             |            |
| **触发条件** | 有需要处理的销售退货单                              |             |            |
| **前置条件** | 进货销售人员必须已经被识别和授权。                        |             |            |
| **后置条件** | 存储销售单的日志，包括操作人、操作时间、操作。                  |             |            |
| **优先级**  | 高                                        |             |            |
| **正常流程** | 1.  选择制定销售退货单。<br />       2. 新建一个销售退货单包括：包括：**单据编号**，**客户**，**业务员**，**操作员**，**仓库，退货商品清单，折让前总额，折让，使用代金卷金额，折让后总额，备注**。退货商品清单中要显示商品的**编号，名称**（从商品选择界面选择）**，**型号**，数量**，单价**（默认为商品信息里的销售价，可修改）**，金额（自动生成），商品备注。3. 进货销售人员确认生成单据。<br />4.系统生成单据，并提示新建成功 |             |            |
| **扩展流程** | 2a.   操作人员遗漏掉某些需要处理的货物信息  <br />1.  系统提示有信息未填，并督促操作人员进行操作<br> 4a.   需要生成的单子超过最大范围（即超过99999条）<br />   1.  系统报错，提示已达到每日生成销售退货单的最大数量 |             |            |
| **特殊需求** | 1.销售单单据格式为XSTHD-yyyyMMdd-xxxxx，后五位每天从1开始编号，所以一天最多可以生成99999条单子<br>2.需要手动输入的信息有数量和商品备注。 |             |            |
...
| **后置条件**    | 保存期初信息                                   |
| **优先级**     | 中                                        |
| **正常流程**    | 1、财务人员填写添加商品信息，客户信息，银行账户信息<br> 2、系统接收消息并且更新存储<br> |
